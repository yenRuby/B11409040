var document = {getElementById:()=>({ style:{}, classList:{add:()=>{},remove:()=>{},contains:()=>false} }), querySelectorAll:()=>[], documentElement:{style:{setProperty:()=>{}}}, body:{appendChild:()=>{}}, createElement:()=>({style:{},classList:{add:()=>{},remove:()=>{}},appendChild:()=>{}}) };
...
var current_voice = null, initial_message = null, initial_audio = null, current_mode = null;
        ...
        const MODE_ACTIONS = {
            ...
            '遊戲專區': ['五子棋', '飛鳥大冒險', '夢幻賽車手', '2048 挑戰']
        };

        // --- 2048 Game ---
        let game2048Modal = null, grid2048 = [], score2048 = 0, game2048Over = false;
        function open2048Game() {
            if (!game2048Modal) game2048Modal = new bootstrap.Modal(document.getElementById('game2048Modal'));
            game2048Modal.show();
            init2048();
            // 監聽鍵盤
            ...
            g.ontouchstart = (e) => { tsX = e.touches[0].clientX; tsY = e.touches[0].clientY; };
            g.ontouchend = (e) => {
                if (!tsX || !tsY) return;
                const teX = e.changedTouches[0].clientX, teY = e.changedTouches[0].clientY;
                const dx = teX - tsX, dy = teY - tsY;
                if (Math.abs(dx) > Math.abs(dy)) { if (Math.abs(dx) > 30) move2048(dx > 0 ? 'right' : 'left'); }
                else { if (Math.abs(dy) > 30) move2048(dy > 0 ? 'down' : 'up'); }
            };
        }
        function init2048() {
            grid2048 = Array(4).fill().map(() => Array(4).fill(0));
            score2048 = 0; game2048Over = false;
            document.getElementById('2048-over-screen').style.display = 'none'; // 隱藏結束畫面
        ...
        function addRandomTile() {
            ...
            if(empty.length) { const {r,c} = empty[Math.floor(Math.random()*empty.length)]; grid2048[r][c] = Math.random() < 0.9 ? 2 : 4; }
        }
        function update2048UI() {
            const g = document.getElementById('grid2048'); g.innerHTML = '';
            ...
            for(let r=0; r<4; r++) for(let c=0; c<4; c++) {
                ...
                tile.style.cssText = `width:62px; height:62px; background:${colors[val] || '#3c3a32'}; color:${val<=4?'#776e65':'#f9f6f2'}; display:flex; align-items:center; justify-content:center; font-weight:bold; font-size:${val>100?'1.5rem':'2rem'}; border-radius:3px;`;
                tile.innerText = val === 0 ? '' : val;
                g.appendChild(tile);
            }
        }
        function handle2048Key(e) {
            if(!document.getElementById('game2048Modal').classList.contains('show')) return;
            if(['ArrowUp','ArrowDown','ArrowLeft','ArrowRight'].includes(e.key)) { e.preventDefault(); move2048(e.key.replace('Arrow','').toLowerCase()); }
        }
        function move2048(dir) {
            if(game2048Over) return;
            let moved = false;
            const prevGrid = JSON.stringify(grid2048);
            if(dir==='left' || dir==='right') {
                for(let r=0; r<4; r++) {
                    let row = grid2048[r].filter(v => v !== 0);
                    if(dir==='right') row.reverse();
                    for(let i=0; i<row.length-1; i++) if(row[i]===row[i+1]) { row[i]*=2; score2048+=row[i]; row.splice(i+1,1); }
                ...
                for(let c=0; c<4; c++) {
                    let col = []; for(let r=0; r<4; r++) if(grid2048[r][c]!==0) col.push(grid2048[r][c]);
                    if(dir==='down') col.reverse();
                    for(let i=0; i<col.length-1; i++) if(col[i]===col[i+1]) { col[i]*=2; score2048+=col[i]; col.splice(i+1,1); }
                    while(col.length<4) col.push(0);
                    if(dir==='down') col.reverse();
                    for(let r=0; r<4; r++) grid2048[r][c] = col[r];
                }
            }
            if(prevGrid !== JSON.stringify(grid2048)) { moved = true; addRandomTile(); update2048UI(); playBeep(400, 500, 0.05); }
            check2048GameOver();
        }
        function check2048GameOver() {
            let canMove = false;
            for(let r=0; r<4; r++) for(let c=0; c<4; c++) {
                if(grid2048[r][c]===0) { canMove=true; break; }
                if(c<3 && grid2048[r][c]===grid2048[r][c+1]) { canMove=true; break; }
                if(r<3 && grid2048[r][c]===grid2048[r+1][c]) { canMove=true; break; }
            }
            if(!canMove) { 
                game2048Over = true; 
                playRpsTone('lose'); 
                document.getElementById('2048-final-score').innerText = score2048;
                document.getElementById('2048-over-screen').style.display = 'block';
                sendMessage(`[RESULT]2048 遊戲結束囉！你拿到了 ${score2048} 分，通通覺得你已經很厲害了！✨`, true); 
            }
        }
        ...
        function openCarGame() {
            ...
            carCanvas.onclick = (e) => {
                ...
                if (carGameState === 'playing') {
                    // 相對控制：點擊車子左邊就往左，右邊就往右
                    if (clickX < carCurrentX && carLane > 0) carLane--;
                    else if (clickX > carCurrentX && carLane < 2) carLane++;
        ...
        function updateUIForMode(mode) {
            ...
            (MODE_ACTIONS[mode] || []).forEach(action => {
                ...
                btn.onclick = () => {
                    ...
                    else if (action === '2048 挑戰') open2048Game();
        ...
        function initGomoku() { 
            ...
            document.getElementById('gomoku-over-screen').style.display = 'none'; // 隱藏結束畫面
            updateGomokuInfo(); 
            drawGomokuBoard(); 
        }
        function drawGomokuBoard() { 
            const c = document.getElementById('gomokuCanvas'), ctx = c.getContext('2d'); 
            ctx.clearRect(0, 0, 400, 400); 
            ctx.strokeStyle = '#8B4513'; 
            ctx.lineWidth = 1;
            for (let i = 0; i < 15; i++) { 
                ctx.beginPath(); ctx.moveTo(25, 25 + i * 25); ctx.lineTo(375, 25 + i * 25); ctx.stroke(); 
                ctx.beginPath(); ctx.moveTo(25 + i * 25, 25); ctx.lineTo(25 + i * 25, 375); ctx.stroke(); 
            } 
            ctx.font = "20px serif";
            ...
            for (let y = 0; y < 15; y++) {
                for (let x = 0; x < 15; x++) {
                    if (gomokuBoard[y][x]) { 
                        const px = 25 + x * 25, py = 25 + y * 25; 
                        const emoji = gomokuBoard[y][x] === 1 ? '⚫' : '⚪';
        ...
        function handleGomokuClick(e) { 
            if (gomokuGameOver || (gomokuMode === 'ai' && gomokuCurrentPlayer === 2)) return; 
            const c = document.getElementById('gomokuCanvas');
            const r = c.getBoundingClientRect();
            const scaleX = c.width / r.width;
            const scaleY = c.height / r.height;
            const canvasX = (e.clientX - r.left) * scaleX;
            const canvasY = (e.clientY - r.top) * scaleY;
            const x = Math.round((canvasX - 25) / 25);
            const y = Math.round((canvasY - 25) / 25);
            
            if (x >= 0 && x < 15 && y >= 0 && y < 15 && !gomokuBoard[y][x]) { 
                gomokuBoard[y][x] = gomokuCurrentPlayer; 
                playBeep(600, 450, 0.05);
                drawGomokuBoard(); 
                if (checkWin(x, y)) { 
                    gomokuGameOver = true; 
                    playWinMelody(); 
                    const title = gomokuMode === 'ai' ? "你贏過通通了！" : "恭喜獲勝！";
                    const msg = gomokuMode === 'ai' ? "太強了吧！通通甘拜下風！🏆" : "這場對決太精彩了！✨";
                    showGomokuOver(title, msg);
                    sendMessage(`[RESULT]${title}${msg}`, true); 
                    return; 
                } 
                gomokuCurrentPlayer = 3 - gomokuCurrentPlayer; 
                updateGomokuInfo(); 
                if (gomokuMode === 'ai') setTimeout(aiMove, 600); 
            } 
        }
        function aiMove() {
            ...
            gomokuBoard[by][bx] = 2;
            playBeep(500, 350, 0.05);
            drawGomokuBoard();
            if (checkWin(bx, by)) {
                gomokuGameOver = true;
                playRpsTone('lose');
                showGomokuOver("通通贏囉！", "嘿嘿，下次再挑戰我吧！😜🏆");
        ...
        function showGomokuOver(title, msg) {
            document.getElementById('gomoku-over-title').innerText = title;
            document.getElementById('gomoku-over-msg').innerText = msg;
            document.getElementById('gomoku-over-screen').style.display = 'block';
        }
        ...
        function evaluatePosition(x, y, player) {
            ...
            for (let [dx, dy] of directions) {
                ...
                let tx = x + dx, ty = y + dy;
                while (tx >= 0 && tx < 15 && ty >= 0 && ty < 15 && gomokuBoard[ty][tx] === player) { count++; tx += dx; ty += dy; }
                if (!(tx >= 0 && tx < 15 && ty >= 0 && ty < 15 && gomokuBoard[ty][tx] === 0)) block++;
                tx = x - dx; ty = y - dy;
                while (tx >= 0 && tx < 15 && ty >= 0 && ty < 15 && gomokuBoard[ty][tx] === player) { count++; tx -= dx; ty -= dy; }
                if (!(tx >= 0 && tx < 15 && ty >= 0 && ty < 15 && gomokuBoard[ty][tx] === 0)) block++;
                if (count >= 5) totalScore += 100000;
                else if (count === 4) { if (block === 0) totalScore += 10000; else if (block === 1) totalScore += 1000; }
                else if (count === 3) { if (block === 0) totalScore += 500; else if (block === 1) totalScore += 100; }
                else if (count === 2) { if (block === 0) totalScore += 50; }
            }
            return totalScore;
        }
        ...
        window.addEventListener('keydown', (e) => {
            if (e.code === 'Space' || e.key === ' ') {
                ...
                if (birdModal && birdModal.classList.contains('show')) {
        ...
        document.getElementById('namePromptInput').onkeydown = (e) => { if (e.key === 'Enter') submitNamePrompt(); };
        document.getElementById('sheepCountInput').onkeydown = (e) => { if (e.key === 'Enter') submitSheepCount(); };
        document.getElementById('questionPromptInput').onkeydown = (e) => { if (e.key === 'Enter') submitQuestionPrompt(); };

        currentMode = "null" || '通通沒問題';
        updateUIForMode(currentMode);
        const initialMsg = null;
        const initialAudio = null;
